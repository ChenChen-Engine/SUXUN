...
public class ConversationSqlManager extends AbstractSQLManager {
    ...
    public static Cursor getConversationCursor() {
        try {
            //String sql = "select unreadCount, im_thread.type, sendStatus, dateTime, sessionId, 5,text, username from im_thread,contacts where im_thread.sessionId = contacts.contact_id order by dateTime desc";
            String sql = "SELECT unreadCount, im_thread.type, sendStatus, dateTime, sessionId, text,im_thread.contactid ,im_thread.isnotice\n" +
                    "      FROM im_thread \n" +
...
//                    "      LEFT JOIN groups2 ON im_thread.sessionId = groups2.groupid order by dateTime desc;";
    ...
    public static boolean hasConversation(String sessionId) {
        String sql = "select sessionId from im_thread where sessionId = '" + sessionId + "'";
    ...
    public static long insertSessionRecord(ECMessage msg) {
        ...
        try {
            ...
            values.put(IThreadColumn.IS_NOTICE, "1");//
    ...
    //群昵称
    public static String queryGrupNickNameSessionId(String contactid) {
        Cursor cursor = null;
            ...
            try {
                ...
                if (cursor != null && cursor.getCount() > 0) {
                    if (cursor.moveToFirst()) {
                        ...
                        groupNickName = cursor.getString(cursor
                                .getColumnIndexOrThrow("groupnickname"));
                    }
                }
            } catch (SQLException e) {
                groupNickName = "";
    ...
    //群昵称
    public static boolean queryIsNoticeBySessionId(String sessionId) {
        Cursor cursor = null;

        boolean isNotice = false;
        if (!TextUtils.isEmpty(sessionId)) {
            String[] columns = new String[]{IThreadColumn.IS_NOTICE};
            String where = IThreadColumn.THREAD_ID + " = '" + sessionId + "' ";
            try {
                cursor = getInstance().sqliteDB().query(
                        DatabaseHelper.TABLES_NAME_IM_SESSION, columns, where,
                        null, null, null, null);
                if (cursor != null && cursor.getCount() > 0) {
                    if (cursor.moveToFirst()) {

                        int columnIndex = cursor.getColumnIndex(IThreadColumn.IS_NOTICE);
                        isNotice = "2".equals(cursor.getString(columnIndex));
                    }
                }
            } catch (SQLException e) {
                isNotice = false;
    ...
    public static void updateSessionChatBg(String sessionId, String bgUri) {

        if (TextUtils.isEmpty(bgUri) || TextUtils.isEmpty(sessionId)) {
            return;
        }
        if (hasConversation(sessionId)) {
            Cursor cursor = null;
            try {

                ContentValues values = new ContentValues();
                values.put("chatbg", bgUri);
                String sql = "select sessionId from " + DatabaseHelper.TABLES_NAME_IM_SESSION + " where sessionId='" + sessionId + "'";
            ...
            ContentValues values = new ContentValues();
            try {
                values.put(IThreadColumn.THREAD_ID, sessionId);
                values.put(IThreadColumn.CHAT_BG, bgUri);
    ...
    // 更新免打扰
    public static long updateSessionIdNotify(String ordinal, String sessionId) {
        ContentValues values = new ContentValues();
        values.put(IThreadColumn.IS_NOTICE, ordinal);
        return getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_IM_SESSION, values, " sessionId='" + sessionId + "'", null);
    }
    public static long updateBg(String sessionId) {
        ContentValues values = new ContentValues();
        values.put(IThreadColumn.CHAT_BG, "");
        return getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_IM_SESSION, values, " sessionId='" + sessionId + "'", null);
    }

    //聊天背景
    public static String queryChatBgSessionId(String sessionId) {
        Cursor cursor = null;

        String chatBg = "";
        if (!TextUtils.isEmpty(sessionId)) {
            try {
                String sql = "select chatbg from " + DatabaseHelper.TABLES_NAME_IM_SESSION + " where sessionId='" + sessionId + "'";
                cursor = getInstance().sqliteDB().rawQuery(sql, null);
                if (cursor != null && cursor.getCount() > 0) {
                    if (cursor.moveToFirst()) {
                        chatBg = cursor.getString(cursor
                                .getColumnIndexOrThrow(IThreadColumn.CHAT_BG));
                    }
                }
            } catch (SQLException e) {
                chatBg = "";
