...
public class ContactSqlManager extends AbstractSQLManager {
     ...
     * 更新联系人名字
     */
    public static void updateContactName(String contactId, String username) {
        ContentValues values = new ContentValues();
        values.put(AbstractSQLManager.ContactsColumn.USERNAME, username);
        getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_CONTACT, values, "contact_id = '" + contactId + "'", null);
    }

    public static void updateContactPhotoUrl(String userId, String url) {

        if (hasContact(userId)) {
            ContentValues values = new ContentValues();
            values.put(FriendColumn.Friend_Pic, url);
            getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_CONTACT, values, " contact_id ='" + userId + "'", null);
        }
    }
    ...
    public static ArrayList<ECContacts> getContacts() {
        ...
        Cursor cursor = null;
        try {
            cursor = getInstance().sqliteDB().query(DatabaseHelper.TABLES_NAME_CONTACT,
                    new String[]{
                            ContactsColumn.ID,
    ...
    public static ECContacts getContact(long rawId) {
        ...
        Cursor cursor = null;
        try {
            cursor = getInstance().sqliteDB().query(DatabaseHelper.TABLES_NAME_CONTACT, new String[]{ContactsColumn.ID, ContactsColumn.USERNAME, ContactsColumn.CONTACT_ID, ContactsColumn.REMARK},
    ...
    public static boolean isExist(String contactId) {
        if (TextUtils.isEmpty(contactId)) {
            return false;
        }
        ECContacts c = new ECContacts(contactId);
    ...
    public static ECContacts getContactLikeUserId(String contactId) {
        if (TextUtils.isEmpty(contactId)) {
            return null;
        }
        Cursor cursor = null;
        try {
            cursor = getInstance().sqliteDB().query(DatabaseHelper.TABLES_NAME_CONTACT, new String[]{ContactsColumn.ID, ContactsColumn.USERNAME, ContactsColumn.CONTACT_ID, ContactsColumn.REMARK},
                    "contact_id LIKE '" + contactId + "'", null, null, null, null, null);
            ECContacts c = null;
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    c = new ECContacts(cursor.getString(2));
