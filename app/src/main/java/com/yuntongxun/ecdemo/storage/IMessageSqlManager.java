...
//        }
        ...
        try {
            if (!TextUtils.isEmpty(message.getSessionId())) {
                ...
                if (contactIds.toUpperCase().startsWith("G")) {//群组
                    GroupSqlManager.checkGroup(contactIds);
                }
                checkContact(message.getForm(), message.getNickName());//联系人
                ownThreadId = ConversationSqlManager.querySessionIdForBySessionId(contactIds);//会话
                if (ownThreadId == 0) {
                    try {
                ...
                if (ownThreadId > 0) {//messsage表
                    ...
                    if (boxType == IMESSENGER_BOX_TYPE_DRAFT) {
                        ...
                        try {
                            values.put(IMessageColumn.OWN_THREAD_ID, ownThreadId);//会话和message表关联
                            values.put(IMessageColumn.MESSAGE_ID, message.getMsgId());
                            values.put(IMessageColumn.SEND_STATUS, message.getMsgStatus().ordinal());
                            values.put(IMessageColumn.READ_STATUS, isread);
                            values.put(IMessageColumn.BOX_TYPE, boxType);
    ...
    private static void putValues(ECMessage message, ContentValues values) {
        if (message.getType() == ECMessage.Type.TXT) {
            ...
            String s =          ((ECTextMessageBody)message.getBody()).getMessage();
            sb.append(s);
            String text  = sb.toString();

            ECTextMessageBody sBody = (ECTextMessageBody)    message.getBody();
            if(message.getForm().startsWith("~ytxro")&&sBody.getRelateArray()!=null){
                 sb.append(",");
                for(String item:sBody.getRelateArray()){
                    sb.append(item+",");
                }
                text = sb.substring(0,sb.length()-1).toString();
            }
            values.put(IMessageColumn.BODY, text);
...
//        ecMessage.setNickName();
    ...
    public static int qureyAllSessionUnreadCount() {
        ...
        try {
            ...
            String where = IThreadColumn.IS_NOTICE + " = '" + 1 + "' ";
            cursor = getInstance().sqliteDB().query(
                    DatabaseHelper.TABLES_NAME_IM_SESSION, columnsList, where,
                    null, null, null, null);
            if (cursor != null && cursor.getCount() > 0) {
                if (cursor.moveToFirst()) {
