...
public class GagMembesFragment extends LazyFrament {
    ...
    private boolean noShowShouXinNum = false;
    @Override
    protected void initView(Bundle savedInstanceState) {
        titleBar.setMyCenterTitle("通讯录");
        mType = getArguments() != null ? getArguments().getInt("type") : TYPE_NORMAL;

        noShowShouXinNum = getActivity().getIntent().getBooleanExtra("NO_SHOUXIN_NUM",false);
        if (positions == null) {
    ...
    private void toNextFriendInfoAct(ECContacts contacts) {
        ...
        intent.putExtra("NO_SHOUXIN_NUM",getActivity().getIntent().getBooleanExtra("NO_SHOUXIN_NUM",false));
