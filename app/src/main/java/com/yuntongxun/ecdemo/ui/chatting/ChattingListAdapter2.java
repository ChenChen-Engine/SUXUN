...
public class ChattingListAdapter2 extends CCPListAdapter<ECMessage> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ECGroup ecGroup = GroupSqlManager.getECGroup(mUsername);
        if(ecGroup!=null){
            chattingRow.setGroupID(ecGroup.getGroupId());
        }else{
            chattingRow.setGroupID("");
        }
...
//            baseHolder.getChattingTime().setBackgroundResource(R.drawable.chat_tips_bg);
            ...
            baseHolder.getChattingTime().setTextColor(Color.parseColor("#adadad"));
        ...
        if(baseHolder.getChattingUser() != null && baseHolder.getChattingUser().getVisibility() == View.VISIBLE) {
            baseHolder.getChattingUser().setTextColor(mChatNameColor[0]);
            baseHolder.getChattingUser().setShadowLayer(0.0F, 0.0F, 0.0F, 0);
        }

        processMessageState(item);

        if(item.getSessionId().equalsIgnoreCase(RestServerDefines.ROBOT)||RestServerDefines.FILE_ASSISTANT.equalsIgnoreCase(item.getSessionId())){
            return chatView;
        }
...
//            }
    ...
    private void  processMessageState(final ECMessage item) {
        if(item!=null&&item.getDirection()== ECMessage.Direction.RECEIVE){
            ...
            if(item.getSessionId().startsWith("~ytx")){
                return;
            }
