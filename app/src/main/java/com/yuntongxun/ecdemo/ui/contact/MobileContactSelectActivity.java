...
public class MobileContactSelectActivity extends BaseActivity implements
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        groupLevel = getIntent().getIntExtra("GROUP_LEVEL", 1);
    ...
    public void onViewClicked() {
        ...
        if (fragments.get(0) instanceof MobileContactFragment) {
            ...
            mGroup = GroupSqlManager.getECGroup(getIntent().getStringExtra("GroupID"));
            if(mGroup!=null){
                ArrayList<ECGroupMember> menbers = GroupMemberSqlManager.getGroupMemberWithName(mGroup.getGroupId());

                if (menbers != null) {
                    switch (groupLevel) {
                        case 1:
                            if (menbers.size() + memberArrs.length > 100) {
                                ToastUtil.showMessage("普通群人数不得大于100");
                                return;
                            }
                        case 2:
                            if (menbers.size() + memberArrs.length > 300) {
                                ToastUtil.showMessage("高级群人数不得大于300");
                                return;
                            }
                        case 3:
                            if (menbers.size() + memberArrs.length > 1000) {
                                ToastUtil.showMessage("VIP群人数不得大于1000");
                                return;
                            }
                    }
                }
