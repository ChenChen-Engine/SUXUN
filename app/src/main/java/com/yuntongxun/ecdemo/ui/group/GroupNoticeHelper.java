...
public class GroupNoticeHelper {
	...
	public static CharSequence getNoticeContent(String content) {
		...
		if(content.indexOf("<admin>") != -1 && content.indexOf("</admin>") != -1) {
			...
			content = content.replace(target, AvatorUtil .getInstance().getMarkName(contactId));
		} 
		if(content.indexOf("<member>") != -1 && content.indexOf("</member>") != -1) {
			...
			content = content.replace(target, AvatorUtil .getInstance().getMarkName(member));
		}
    ...
    public static void insertNoticeMessage(ECGroupNoticeMessage instanceMsg ,OnPushGroupNoticeMessageListener l)  {
		...
		if(type == ECGroupNoticeMessage.ECGroupMessageType.JOIN) {// 直接加入
			...
			// TODO: 2017/8/30   确认加入后
			ECGroupMember member = new ECGroupMember();
			member.setDisplayName(rInviteMsg.getNickName());
			member.setRole(ECGroupMember.Role.MEMBER);
			member.setBan(false);
			member.setVoipAccount(rInviteMsg.getMember());

			if(rInviteMsg.getConfirm() == 2){
				CCPAppManager.sendAddBR(member);
			}
			...
			CCPAppManager.sendChangeAdminBR();
