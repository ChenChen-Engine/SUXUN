...
public class FriendInfoUI extends BaseActivity {
    ...
    void noManager() {
        ...
        if (!TextUtils.isEmpty(groupID)) {
            ECGroupMember.Role role = ECGroupMember.Role.values()[GroupMemberSqlManager.getSelfRoleWithGroupId(groupID, CCPAppManager.getUserId()) - 1];
            if (role == ECGroupMember.Role.MEMBER) {
                /*普通群员，不显示添加好友按钮*/
...
//        btnAdd.setVisibility(View.VISIBLE);
    ...
    protected void initView(Bundle savedInstanceState) {
        if (getIntent().getBooleanExtra("NO_SHOUXIN_NUM", false)) {
            tvMyPhone.setVisibility(View.GONE);
        }
        ...
        if (TextUtils.equals(CCPAppManager.getUserId(), mobile)) {
            ...
            if (friend != null) {
                ...
                if (!FriendMessageSqlManager.isFriend(mobile)) {
                    noFriend();
                }
            }
        }
    ...
    private void getGroupLevel(String groupID) {
        if (TextUtils.isEmpty(groupID)) {
            return;
        }
        Net.getGroupLevel(groupID, new BaseObserver<GetGroupLevelBean>() {
            ...
            public void onNext(GetGroupLevelBean value) {
                if (value != null && value.getData() != null) {
                    if (value.getData().getGroup_level() == 1) {
                        btnAdd.setVisibility(View.VISIBLE);
                    } else {
                        btnAdd.setVisibility(View.GONE);
                    }
